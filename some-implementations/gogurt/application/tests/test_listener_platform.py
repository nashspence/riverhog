...
def test_launchd_stop_waits_until_the_native_job_is_unloaded(
    ...
    monkeypatch.setattr("a_gogurt_macos_listener.time.sleep", lambda _seconds: None)
...
def test_windows_stop_waits_for_cooperative_listener_settlement(
    ...
    monkeypatch.setattr("a_gogurt_windows_listener.time.sleep", lambda _seconds: None)
...
def test_windows_stop_fails_closed_without_hard_termination(
    ...
    monkeypatch.setattr("a_gogurt_windows_listener.time.monotonic", lambda: next(times))
    monkeypatch.setattr("a_gogurt_windows_listener.time.sleep", lambda _seconds: None)
