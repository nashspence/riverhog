...
from a_review0_rclone_target import (
    RcloneReviewDestination,
    ReviewRcloneEffectTargetService,
)
from a_review0_rclone_target import app as effect_app
from a_review0_rclone_target import target as effect_target
...
def test_review_process_exposes_only_target_descriptor(tmp_path: Path) -> None:
    ...
    with TestClient(create_app(token="review-secret", target=target)) as client:
        ...
        assert response.json()["implementation_id"] == "a-review0-materializer/v1"
        assert (
            client.get(
                "/v1/sampler",
                headers={"Authorization": "Bearer review-secret"},
...
def test_rclone_review_destination_commits_manifest_last_and_returns_opaque_receipt(
    ...
    manifest.write_bytes(b'{"format":"review0-delivery-manifest/v1"}')
    calls: list[list[str]] = []
...
def test_review_effect_executes_sampling_delivery_and_canonical_receipt_end_to_end(
    ...
    class RenderingSampler(FixtureSamplerClient):
        def sample(self, request: object) -> SamplerResult:
            from review0_sampler_protocol import SamplerRequest
