"""Private FFmpeg helpers for the NVENC AV1 plus Opus review sampler."""
...
class NvencContentError(RuntimeError):
    pass
...
def run_ffmpeg(
        ...
        with log.open("wb") as errors:
            ...
            try:
                while process.poll() is None:
                    if canceled():
                        raise InterruptedError("NVENC AV1 plus Opus review sampling was canceled")
    ...
    if process.returncode:
        raise NvencContentError(detail or f"FFmpeg exited with status {process.returncode}")
...
__all__ = ["NvencContentError", "file_identity", "run_ffmpeg", "tool_version"]
