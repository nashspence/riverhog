...
SERVICE = "a-stove0-exiftool-observer"
...
def create_app(*, token: str, observer: ExiftoolObserver) -> FastAPI:
    ...
    def ready() -> Response:
        ...
        return Response(
            content=b'{"service":"a-stove0-exiftool-observer","status":"ok"}',
...
def _secret() -> str:
    direct = os.getenv("A_STOVE0_EXIFTOOL_OBSERVER_TOKEN")
    path = os.getenv("A_STOVE0_EXIFTOOL_OBSERVER_TOKEN_FILE")
...
_CLI_RESULT_CONTRACT = {
    ...
    "profiles": {
        "runtime": {
            "id": "a-stove0-exiftool-observer-cli-runtime/v1",
    ...
    "output_authorities": {},
    "version_distribution": "a-stove0-exiftool-observer",
}
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--host", default=os.getenv("A_STOVE0_EXIFTOOL_OBSERVER_HOST", "127.0.0.1"))
    parser.add_argument(
        "--port", type=int, default=int(os.getenv("A_STOVE0_EXIFTOOL_OBSERVER_PORT", "8080"))
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    observer = ExiftoolObserver(
        ...
        source_revision=os.getenv("A_STOVE0_EXIFTOOL_OBSERVER_SOURCE_REVISION", "unknown"),
...
def _image_digest() -> str:
    value = os.getenv("A_STOVE0_EXIFTOOL_OBSERVER_IMAGE_DIGEST", "").strip()
    if len(value) != 64 or any(character not in "0123456789abcdef" for character in value):
        raise ValueError("A_STOVE0_EXIFTOOL_OBSERVER_IMAGE_DIGEST must be a lowercase SHA-256")
    return value
