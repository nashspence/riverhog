...
def test_a_stove0_client_preserves_the_public_wire_error() -> None:
    app = create_app(_composition())
...
def test_a_stove0_client_rejects_undeclared_operation_error_pairs(
    operation_id: str,
    status: int,
    code: str,
) -> None:
...
def test_a_stove0_client_transport_configuration_is_connected(
...
def test_installed_conformance_catalog_is_exact_through_api_client_and_cli(
    ...
    with TestClient(application) as transport:
        ...
        monkeypatch.setattr(a_stove0_cli, "Stove0ApiClient", cli_client)
        runner = CliRunner()
        human = runner.invoke(
            a_stove0_cli.app,
