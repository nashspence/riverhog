...
def test_work_list_rich_and_json_preserve_effect_result_identity(
    ...
    monkeypatch.setattr(a_stove0_cli, "Stove0ApiClient", FakeClient)
    runner = CliRunner()
    human = runner.invoke(a_stove0_cli.app, ["work", "list"])
    assert human.exit_code == 0
    assert "external-eff" in human.stdout
    assert "succeeded" in human.stdout
    assert "a" * 12 in human.stdout

    machine = runner.invoke(a_stove0_cli.app, ["--json", "work", "list"])
...
def test_work_list_rich_projection_identifies_coordination_settlement() -> None:
    ...
    assert a_stove0_cli._table_value(row, "result_kind") == "coordination"
    assert a_stove0_cli._table_value(row, "result_identity") == "c" * 64
