...
class FtpSpoolApiError(RuntimeError):
    def __init__(
        self, message: str, *, code: str = "ftp_spool_error", status: int | None = None
    ) -> None:
...
class RiverhogFtpSpoolClient:
    """Conventional authenticated client for one FTP spool deployment."""
    ...
    def __init__(
        ...
        *,
        ...
        allow = (
            _bool_env("A_RIVERHOG_FTP_SPOOL_ALLOW_INSECURE_HTTP", False)
            if allow_insecure_http is None
            else allow_insecure_http
        )
        self.base_url = safe_http_base_url(
            base_url or os.getenv("A_RIVERHOG_FTP_SPOOL_BASE_URL") or "http://127.0.0.1:8082",
            setting="A_RIVERHOG_FTP_SPOOL_BASE_URL",
            allow_insecure_http=allow,
        )
        self.allow_insecure_http = allow
        self.token = token or os.getenv("A_RIVERHOG_FTP_SPOOL_TOKEN")
        self.http2 = _bool_env("A_RIVERHOG_FTP_SPOOL_HTTP2", True) if http2 is None else http2
        self.timeout_seconds = (
            _timeout_env("A_RIVERHOG_FTP_SPOOL_HTTP_TIMEOUT_SECONDS", 300.0)
    ...
    def flush_ftp_spool_source(self, source_id: str) -> dict[str, Any]:
    ...
    def _json(
        ...
        *,
        ...
        if authenticated and not self.token:
            raise FtpSpoolApiError("A_RIVERHOG_FTP_SPOOL_TOKEN is required", code="unauthorized")
        response = self._http.request(method, path, params=params)
        self._raise(response)
        payload = response.json()
        if not isinstance(payload, dict):
            raise FtpSpoolApiError(
                "FTP spool returned a non-object response", code="invalid_response"
            )
        return payload
    ...
    def _raise(response: httpx.Response) -> None:
        ...
        raise FtpSpoolApiError(message, code=code, status=response.status_code)
...
__all__ = ["FtpSpoolApiError", "RiverhogFtpSpoolClient"]
