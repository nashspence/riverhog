...
from a_riverhog_ftp_spool.state_contract import (
...
class SourceChanged(FtpSpoolError):
    pass


class ClaimCollision(FtpSpoolError):
    pass
...
class FtpSpool:
    ...
    def _run_once(self, source_ids: Sequence[str] | None = None) -> dict[str, object]:
                    ...
                    if admission.claim_root is not None:
                        ...
                        try:
                            ...
                            if admission.claim_ordinal is None:
                                raise FtpSpoolError("new FTP claim has no durable ordinal")
        ...
        return {
            "format": "a-riverhog-ftp-spool-pass/v1",
            "completed": completed,
            "failed": failed,
    ...
    def flush(self, source_id: str) -> dict[str, object]:
        ...
        with self._custody_pass_lock:
            ...
            _write_atomic(marker, b"a-riverhog-ftp-spool-flush/v1\n")
    ...
    def _status(
        ...
        *,
        ...
        if page_size < 1 or page_size > 100:
            raise ValueError("FTP spool status page_size must be in 1..100")
    ...
    def _accept_completed_file(
        ...
        *,
        ...
        if claim_count >= self.config.pending_claim_capacity:
            raise FtpSpoolError("pending claim capacity is exhausted; admission is deferred")
    ...
    def _claim_summary(self, source: SourceConfig) -> tuple[int, int]:
        with closing(self._open_state(source)) as connection:
            ...
            claim_bytes = _state_value(connection, "claim_bytes")
        if claim_count is None or claim_bytes is None:
            raise FtpSpoolError("FTP claim accounting is unavailable")
    ...
    def _completion_cursor(self, source: SourceConfig) -> tuple[str, int]:
        ...
        if generation is None or raw_offset is None:
            raise FtpSpoolError("FTP completion cursor is unavailable")
        ...
        *,
        ...
        with closing(self._open_state(source)) as connection:
            ...
            if current_generation != generation:
                raise FtpSpoolError("FTP completion authority changed during traversal")
            if current_offset is None or offset < int(current_offset):
                raise FtpSpoolError("FTP completion cursor moved backwards")
    ...
    def _completion_failure_count(self, source: SourceConfig) -> int:
        ...
        if value is None:
            raise FtpSpoolError("FTP completion failure accounting is unavailable")
        return int(value)
    ...
    def _reclaim_completion_events(self, source: SourceConfig, *, limit: int) -> int:
        ...
        with self._completion_log_path(source).open("rb") as stream:
            ...
            try:
                ...
                if current_generation != generation:
                    raise FtpSpoolError("FTP completion authority changed during reclamation")
        ...
        *,
        ...
        with closing(self._open_state(source)) as connection:
            ...
            if (
                ...
                raise FtpSpoolError("FTP completion failure cursor changed during admission")
            existing = connection.execute(
        ...
        with closing(self._open_state(source)) as connection:
            ...
            if existing is None:
                ...
                if cursor.lastrowid is None:
                    raise FtpSpoolError("FTP claim registration returned no durable ordinal")
            ...
            if not isinstance(raw_event_ids, list) or any(
                ...
                raise FtpSpoolError("FTP claim completion event identities are invalid")
            ...
            if discovery_generation is not None or discovery_offset is not None:
                if discovery_generation is None or discovery_offset is None:
                    raise FtpSpoolError("incomplete FTP completion cursor update")
                current_generation = _state_value(connection, "completion_generation")
    ...
    def _registered_claim_manifest(
        ...
        if record is None:
            raise FtpSpoolError(f"FTP claim is not registered: {claim_id}")
        ...
        try:
            ...
            raise FtpSpoolError("registered FTP claim manifest is invalid") from exc
        if payload["claim_id"] != claim_id or payload["source"] != source.id:
            raise FtpSpoolError("registered FTP claim manifest is invalid")
        return payload
        ...
        with closing(self._open_state(source)) as connection:
            ...
            if row is not None:
                ...
                if next_count < 0 or next_bytes < 0:
                    raise FtpSpoolError("FTP claim accounting is inconsistent")
    ...
    def _finish_claim_cleanup(self, source: SourceConfig, claim_id: str) -> None:
        ...
        if record is None:
            if claim_root.exists():
                raise FtpSpoolError("finalized FTP claim has files but no durable manifest")
            return
    ...
    def _discover_batch(self, source: SourceConfig, *, flush: bool) -> _DiscoveryBatch:
        ...
        with log_path.open("rb") as stream:
            ...
            try:
                if os.fstat(stream.fileno()).st_size < offset:
                    raise FtpSpoolError("FTP completion log ended before its durable cursor")
                stream.seek(offset)
    ...
    def _publish_claim_once(self, source: SourceConfig, claim_root: Path) -> ProducedCollection:
        ...
        receipt_payload = {
            "format": "a-riverhog-ftp-spool-receipt/v1",
...
def _require_identity(current: os.stat_result, row: Mapping[str, object], path: str) -> None:
    ...
    if actual != expected or not stat.S_ISREG(current.st_mode):
        raise SourceChanged(f"FTP spool source changed during claim: {path}")
...
def _read_manifest(claim_root: Path) -> dict[str, object]:
    try:
        ...
        raise FtpSpoolError(f"invalid FTP spool claim: {claim_root}") from exc
...
def _file_rows(manifest: Mapping[str, object]) -> list[dict[str, object]]:
    ...
    if not isinstance(value, list) or not value:
        raise FtpSpoolError("FTP spool claim has no files")
