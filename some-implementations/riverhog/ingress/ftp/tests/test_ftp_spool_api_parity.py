...
import httpx
from a_riverhog_ftp_spool import app as adapter_app
...
def _composition(tmp_path: Path) -> FtpSpoolComposition:
    ...
    return FtpSpoolComposition(config, _Riverhog(), _Adapter())  # type: ignore[arg-type]
...
def test_public_openapi_has_one_stable_operation_for_each_official_client_method(
    ...
    assert operations <= set(dir(RiverhogFtpSpoolClient))
...
def test_official_client_positive_disposable_lifecycle(tmp_path: Path) -> None:
    ...
    observer = OperationObserver.install(application, application="a-riverhog-ftp-spool")
...
class _OperatorClient:
    ...
    def flush_ftp_spool_source(self, source_id: str) -> dict[str, object]:
...
def test_operator_cli_has_human_and_json_views_for_each_management_operation(
    ...
    monkeypatch.setattr(adapter_app, "RiverhogFtpSpoolClient", _OperatorClient)
