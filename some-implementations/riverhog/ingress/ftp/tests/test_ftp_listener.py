...
import a_riverhog_ftp_spool.listener as ftp_listener
import pytest
...
from a_riverhog_ftp_spool.listener import build_ftp_server
    ...
    with _listener(root) as address:
        ...
        try:
            ...
            while time.monotonic() < deadline:
                ...
                if any(
                    line.strip() == f"Total bytes received: {len(prefix)}"
...
def test_same_path_replay_and_new_event_partition_into_restartable_claims(
    ...
    first_pass = FtpSpool(object(), config).run_once()  # type: ignore[arg-type]
    second_pass = FtpSpool(object(), config).run_once()  # type: ignore[arg-type]
...
def test_listener_recovers_published_intent_after_exact_adapter_acquisition(
    ...
    monkeypatch.setattr(landing, "CollectionProducer", _Producer)
    adapter_result = FtpSpool(object(), config).run_once()  # type: ignore[arg-type]
    ...
    if not finalize_before_listener_restart:
        _Producer.available = True
        settled = FtpSpool(object(), config).run_once()  # type: ignore[arg-type]
        assert settled["completed"] == 1
    assert list((root / CONTROL_DIR / "handoffs").glob("*/acquired-*.json")) == []
    FtpSpool(object(), config).run_once()  # type: ignore[arg-type]
    matching_calls = [
        call
