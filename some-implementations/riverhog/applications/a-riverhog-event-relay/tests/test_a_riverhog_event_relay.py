...
import a_riverhog_event_relay.relay as relay_module
import httpx
import pytest
...
def test_a_riverhog_event_relay_state_commands_report_and_verify_the_current_revision(
    tmp_path: Path,
    ...
    assert a_riverhog_event_relay_main([*prefix, "status", "--json"]) == 0
    empty = json.loads(capsys.readouterr().out)
    assert a_riverhog_event_relay_main([*prefix, "upgrade", "--json"]) == 0
    upgraded = json.loads(capsys.readouterr().out)
    assert a_riverhog_event_relay_main([*prefix, "verify", "--json"]) == 0
...
def test_relative_state_path_is_resolved_from_config_for_every_command(
    ...
    expected_state = config_path.parent / "state" / "a-riverhog-event-relay.sqlite3"

    config = load_config(config_path)
    assert config.state_path == expected_state
    assert a_riverhog_event_relay_main(["--config", str(config_path), "--check"]) == 0
    ...
    assert a_riverhog_event_relay_main(["--config", str(config_path), "--once"]) == 0
...
def test_one_shot_partial_failure_is_visible_and_does_not_block_other_sources(
    ...
    assert a_riverhog_event_relay_main(["--config", str(config_path), "--once"]) == 1
...
def test_a_riverhog_event_relay_advances_only_after_the_complete_page_is_delivered(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("EVENT_TOKEN", "token")
