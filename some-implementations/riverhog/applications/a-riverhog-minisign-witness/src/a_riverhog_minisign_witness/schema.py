"""The Minisign witness owns one SQLite state and one forward migration chain."""
...
statements = Table(
    ...
    Column("key_identity", Text),
)
Index("ix_statements_due", statements.c.state, statements.c.due, statements.c.digest)
...
def state_schema(database: Path) -> StateSchema:
    ...
    return StateSchema(
        name="a-riverhog-minisign-witness",
