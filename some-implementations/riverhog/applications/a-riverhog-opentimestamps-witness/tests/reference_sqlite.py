...
from a_riverhog_opentimestamps_witness.proof import Job, StateError, dump_job, load_job
...
class Store:
        ...
        with self.connection:
            ...
            if cursor.rowcount and job.proof is not None:
                self.connection.execute(
                    "INSERT INTO proofs VALUES (?, 0, ?)", (job.statement_digest, job.proof)
                )
    ...
    def save(self, expected_revision: int, job: Job) -> int:
        ...
        with self.connection:
            ...
            if (
                previous.nonce != job.nonce
                or previous.submit_urls != job.submit_urls
