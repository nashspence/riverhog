...
"""Bounded OpenTimestamps proof work for the supplied external witness application.
...
from bitcoin.core import CBlockHeader  # type: ignore[import-untyped]
from opentimestamps.core.notary import (  # type: ignore[import-untyped]
    BitcoinBlockHeaderAttestation,
    PendingAttestation,
    VerificationError,
)
from opentimestamps.core.op import OpAppend, OpSHA256  # type: ignore[import-untyped]
...
class StateError(ValueError):
    """Corrupt or unsupported local witness state."""
...
class _Context(BytesDeserializationContext):  # type: ignore[misc]
    ...
    def read_bytes(self, expected_length: int | None = None) -> bytes:
        ...
        return cast(bytes, super().read_bytes(expected_length))

    def read_varuint(self) -> int:
        value = 0
        for shift in range(0, 70, 7):
            byte = cast(bytes, self.fd_read(1))[0]
...
def validate_job(job: Job) -> None:
    try:
        ...
        digest32(job.nonce)
        if (
            type(job.submit_urls) is not tuple
            or not 1 <= len(job.submit_urls) <= MAX_WORK
        ...
        for url in job.submit_urls:
            calendar_url(url)
        tips = (
            pending(read_proof(job.proof, job.statement_digest)) if job.proof is not None else set()
        )
        ...
        for work in job.work:
            ...
            if (
                type(work.commitment) is not bytes
                or not 1 <= len(work.commitment) <= 4096
            ...
            keys.add(work.key)
            if work.kind == "submit":
                if work.url not in job.submit_urls or work.commitment != job.submission_commitment:
        ...
        raise StateError("invalid witness job") from exc
...
def enable_calendars(job: Job, *, allow: frozenset[str], now: int) -> Job:
    """Purely discover newly allowed pending paths without a calendar exchange."""
    dump_job(job)
    if not integer(now) or type(allow) is not frozenset:
        raise ValueError("invalid scheduler inputs")
    for url in allow:
        calendar_url(url)
    result = _discover(job, allow, now)
    dump_job(result)
    return result


def step(
    job: Job, calendar: Calendar, *, now: int, allow: frozenset[str], retry: Retry = DEFAULT_RETRY
) -> Job:
    ...
    """
    job = enable_calendars(job, allow=allow, now=now)
    ...
    try:
        ...
        if selected.kind == "submit":
            ...
            targets = [
                node
                for node in nodes(root)
                if node.msg == selected.commitment
    ...
    dump_job(result)  # Never return progress that the witness store cannot serialize.
    return result
...
def dump_job(job: Job) -> bytes:
    ...
    body = {
        "version": 1,
        "statement_digest": job.statement_digest.hex(),
        "nonce": job.nonce.hex(),
        "submit_urls": list(job.submit_urls),
...
def load_job(data: bytes, expected_digest: bytes) -> Job:
    ...
    try:
        ...
        if (
            ...
            or set(body) != {"version", "statement_digest", "nonce", "submit_urls", "proof", "work"}
        ...
        work = tuple(
            Work(**{**w, "commitment": bytes.fromhex(w["commitment"])}) for w in body["work"]
        )
        job = Job(
            bytes.fromhex(body["statement_digest"]),
            bytes.fromhex(body["nonce"]),
            tuple(body["submit_urls"]),
            None if body["proof"] is None else bytes.fromhex(body["proof"]),
            work,
        )
        ...
        raise StateError("invalid witness proof state") from exc
...
def verify(
    proof: bytes, statement_digest: bytes, bitcoin: Bitcoin, *, max_heights: int = 8
) -> Verification:
    ...
    for message, att in attestations:
        ...
        if (
            not integer(anchor.height)
            or anchor.height != height
            or type(anchor.header) is not bytes
            or len(anchor.header) != 80
        ...
        try:
            ...
            checks.append(
                Check(
                    "valid",
                    height,
                    "bitcoin_merkle_commitment",
                    anchor.block_hash,
                    block_time,
                    anchor.tip_height - height + 1,
                    anchor.tip_hash,
                )
            )
    return Verification(
        hashlib.sha256(proof).hexdigest(), tuple(checks), pending_count, unsupported_count
    )
...
def confirmation_policy(result: Verification, minimum: int | None = None) -> bool:
    ...
    return any(
        check.status == "valid"
        and (
            minimum is None or (check.confirmations is not None and check.confirmations >= minimum)
        )
