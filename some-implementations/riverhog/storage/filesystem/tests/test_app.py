...
def test_config_from_environment(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("A_RIVERHOG_FILESYSTEM_STORE_SEGMENT_BYTES", "128KiB")
    monkeypatch.setenv("A_RIVERHOG_FILESYSTEM_STORE_READ_CHUNK_BYTES", "1MiB")
...
def test_root_must_be_absolute_non_root(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("A_RIVERHOG_FILESYSTEM_STORE_ROOT", "/")
...
def test_secret_requires_exactly_one_source(
    ...
    monkeypatch.delenv("A_RIVERHOG_FILESYSTEM_STORE_TOKEN", raising=False)
    monkeypatch.delenv("A_RIVERHOG_FILESYSTEM_STORE_TOKEN_FILE", raising=False)
