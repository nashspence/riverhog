# riverhog-age: Python

[Atlas](../../../index.md) · [Authority](../index.md) · [Policies](../../../policies/index.md)

Declared public imports and their selected exact structural contracts.

## Semantic dossiers

### `riverhog_age`

- [riverhog_age.AEAD_TAG_SIZE](riverhog-age-aead-tag-size.md)
- [riverhog_age.AgeAlignedUnitPlan](riverhog-age-agealignedunitplan.md)
  - [riverhog_age.AgeAlignedUnitPlan.ciphertext_len](riverhog-age-agealignedunitplan-ciphertext-len.md)
  - [riverhog_age.AgeAlignedUnitPlan.plaintext_len](riverhog-age-agealignedunitplan-plaintext-len.md)
- [riverhog_age.AgeDecryptError](riverhog-age-agedecrypterror.md)
- [riverhog_age.AgeFormatError](riverhog-age-ageformaterror.md)
- [riverhog_age.CHUNK_SIZE](riverhog-age-chunk-size.md)
- [riverhog_age.DEFAULT_CHUNKS_PER_AGE_UNIT](riverhog-age-default-chunks-per-age-unit.md)
- [riverhog_age.DEFAULT_SCRYPT_LOG_N](riverhog-age-default-scrypt-log-n.md)
- [riverhog_age.PAYLOAD_NONCE_SIZE](riverhog-age-payload-nonce-size.md)
- [riverhog_age.ResumableAgeScryptSession](riverhog-age-resumableagescryptsession.md)
  - [riverhog_age.ResumableAgeScryptSession.age_aligned_unit_plans](riverhog-age-resumableagescryptsession-age-aligned-unit-plans.md)
  - [riverhog_age.ResumableAgeScryptSession.age_prefix](riverhog-age-resumableagescryptsession-age-prefix.md)
  - [riverhog_age.ResumableAgeScryptSession.create](riverhog-age-resumableagescryptsession-create.md)
  - [riverhog_age.ResumableAgeScryptSession.decrypt_chunk](riverhog-age-resumableagescryptsession-decrypt-chunk.md)
  - [riverhog_age.ResumableAgeScryptSession.encrypt_chunk](riverhog-age-resumableagescryptsession-encrypt-chunk.md)
  - [riverhog_age.ResumableAgeScryptSession.encrypt_part](riverhog-age-resumableagescryptsession-encrypt-part.md)
  - [riverhog_age.ResumableAgeScryptSession.encrypt_plaintext](riverhog-age-resumableagescryptsession-encrypt-plaintext.md)
  - [riverhog_age.ResumableAgeScryptSession.export_state](riverhog-age-resumableagescryptsession-export-state.md)
  - [riverhog_age.ResumableAgeScryptSession.from_state](riverhog-age-resumableagescryptsession-from-state.md)
- [riverhog_age.UploadState](riverhog-age-uploadstate.md)
  - [riverhog_age.UploadState.from_json_bytes](riverhog-age-uploadstate-from-json-bytes.md)
  - [riverhog_age.UploadState.to_json_bytes](riverhog-age-uploadstate-to-json-bytes.md)
- [riverhog_age.age_chunk_count_for_plaintext_len](riverhog-age-age-chunk-count-for-plaintext-len.md)
- [riverhog_age.age_ciphertext_len_for_plaintext_len](riverhog-age-age-ciphertext-len-for-plaintext-len.md)
- [riverhog_age.decrypt_age_scrypt](riverhog-age-decrypt-age-scrypt.md)
- [riverhog_age.encrypt_age_scrypt](riverhog-age-encrypt-age-scrypt.md)
- [riverhog_age.iter_decrypt_age_scrypt](riverhog-age-iter-decrypt-age-scrypt.md)
- [riverhog_age.iter_decrypt_payload_chunks](riverhog-age-iter-decrypt-payload-chunks.md)
- [riverhog_age.make_age_aligned_unit_plans](riverhog-age-make-age-aligned-unit-plans.md)
- [riverhog_age.parse_scrypt_header](riverhog-age-parse-scrypt-header.md)
- [riverhog_age.parse_scrypt_header_from_age_file](riverhog-age-parse-scrypt-header-from-age-file.md)
- [riverhog_age.plaintext_bytes_for_ciphertext_offset](riverhog-age-plaintext-bytes-for-ciphertext-offset.md)
- [riverhog_age.split_plaintext_chunks](riverhog-age-split-plaintext-chunks.md)
