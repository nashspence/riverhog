# gogurt-path-volume-support: Python

[Atlas](../../../index.md) · [Authority](../index.md) · [Policies](../../../policies/index.md)

Declared public imports and their selected exact structural contracts.

## Semantic dossiers

### `gogurt_path_volume_support`

- [gogurt_path_volume_support.MAX_PATH_MARKER_BYTES](gogurt-path-volume-support-max-path-marker-bytes.md)
- [gogurt_path_volume_support.PATH_MARKER_NAME](gogurt-path-volume-support-path-marker-name.md)
- [gogurt_path_volume_support.PORTABLE_MARKER_MODE](gogurt-path-volume-support-portable-marker-mode.md)
- [gogurt_path_volume_support.PathMountedVolumeAccess](gogurt-path-volume-support-pathmountedvolumeaccess.md)
  - [gogurt_path_volume_support.PathMountedVolumeAccess.discover](gogurt-path-volume-support-pathmountedvolumeaccess-discover.md)
  - [gogurt_path_volume_support.PathMountedVolumeAccess.observe_marker](gogurt-path-volume-support-pathmountedvolumeaccess-observe-marker.md)
  - [gogurt_path_volume_support.PathMountedVolumeAccess.publish_marker](gogurt-path-volume-support-pathmountedvolumeaccess-publish-marker.md)
