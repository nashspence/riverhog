# gogurt-path-volume-support: Python

[Atlas](../../../index.md) · [Authority](../index.md) · [Policies](../../../policies/index.md)

Declared public imports and their selected exact structural contracts.

## Semantic dossiers

### `gogurt_path_volume_support`

- [MAX_PATH_MARKER_BYTES](gogurt-path-volume-support-max-path-marker-bytes.md)
- [PATH_MARKER_NAME](gogurt-path-volume-support-path-marker-name.md)
- [PORTABLE_MARKER_MODE](gogurt-path-volume-support-portable-marker-mode.md)
- [PathMountedVolumeAccess](gogurt-path-volume-support-pathmountedvolumeaccess.md)
  - [discover](gogurt-path-volume-support-pathmountedvolumeaccess-discover.md)
  - [observe_marker](gogurt-path-volume-support-pathmountedvolumeaccess-observe-marker.md)
  - [publish_marker](gogurt-path-volume-support-pathmountedvolumeaccess-publish-marker.md)
