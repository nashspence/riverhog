# stove0-review-target-support: Python

[Atlas](../../../index.md) · [Authority](../index.md) · [Policies](../../../policies/index.md)

Declared public imports and their selected exact structural contracts.

## Semantic dossiers

### `stove0_review_target_support`

- [ReviewTargetConfig](stove0-review-target-support-reviewtargetconfig.md)
  - [canonical_samplers](stove0-review-target-support-reviewtargetconfig-canonical-samplers.md)
- [ReviewTargetServiceBase](stove0-review-target-support-reviewtargetservicebase.md)
  - [cancel_job](stove0-review-target-support-reviewtargetservicebase-cancel-job.md)
  - [close](stove0-review-target-support-reviewtargetservicebase-close.md)
  - [contract](stove0-review-target-support-reviewtargetservicebase-contract.md)
  - [get_job](stove0-review-target-support-reviewtargetservicebase-get-job.md)
  - [preflight](stove0-review-target-support-reviewtargetservicebase-preflight.md)
  - [prune_terminal_state](stove0-review-target-support-reviewtargetservicebase-prune-terminal-state.md)
  - [put_job](stove0-review-target-support-reviewtargetservicebase-put-job.md)
  - [readiness](stove0-review-target-support-reviewtargetservicebase-readiness.md)
- [SamplerConfig](stove0-review-target-support-samplerconfig.md)
  - [absolute_token_file](stove0-review-target-support-samplerconfig-absolute-token-file.md)
- [SamplerRegistration](stove0-review-target-support-samplerregistration.md)
  - [descriptor](stove0-review-target-support-samplerregistration-descriptor.md)
- [create_target_app](stove0-review-target-support-create-target-app.md)
- [file_identity](stove0-review-target-support-file-identity.md)
- [load_sampler_registrations](stove0-review-target-support-load-sampler-registrations.md)
- [parse_sampler_registrations](stove0-review-target-support-parse-sampler-registrations.md)
- [review_options_schema](stove0-review-target-support-review-options-schema.md)
