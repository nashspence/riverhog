# stove0-target-protocol: Python

[Atlas](../../../index.md) · [Authority](../index.md) · [Policies](../../../policies/index.md)

Declared public imports and their selected exact structural contracts.

## Contract elements

### `stove0_target_protocol`

- [ARTIFACT_ID_PATTERN](stove0-target-protocol-artifact-id-pattern.md)
- [AcceptedTargetJob](stove0-target-protocol-acceptedtargetjob.md)
  - [verify_digest](stove0-target-protocol-acceptedtargetjob-verify-digest.md)
- [DEPARTURE_EFFECT_HTTP_OPERATIONS](stove0-target-protocol-departure-effect-http-operations.md)
- [DepartureEffectIntent](stove0-target-protocol-departureeffectintent.md)
  - [exact_identity](stove0-target-protocol-departureeffectintent-exact-identity.md)
  - [later_than_last_seen](stove0-target-protocol-departureeffectintent-later-than-last-seen.md)
  - [seal](stove0-target-protocol-departureeffectintent-seal.md)
- [DepartureEffectIntentPayload](stove0-target-protocol-departureeffectintentpayload.md)
  - [later_than_last_seen](stove0-target-protocol-departureeffectintentpayload-later-than-last-seen.md)
- [DepartureEffectReceipt](stove0-target-protocol-departureeffectreceipt.md)
  - [bounded_result](stove0-target-protocol-departureeffectreceipt-bounded-result.md)
  - [exact_identity](stove0-target-protocol-departureeffectreceipt-exact-identity.md)
  - [seal](stove0-target-protocol-departureeffectreceipt-seal.md)
- [DepartureEffectReceiptPayload](stove0-target-protocol-departureeffectreceiptpayload.md)
  - [bounded_result](stove0-target-protocol-departureeffectreceiptpayload-bounded-result.md)
- [DepartureEffectTargetDescriptor](stove0-target-protocol-departureeffecttargetdescriptor.md)
  - [exact_identity](stove0-target-protocol-departureeffecttargetdescriptor-exact-identity.md)
  - [seal](stove0-target-protocol-departureeffecttargetdescriptor-seal.md)
- [DepartureEffectTargetDescriptorPayload](stove0-target-protocol-departureeffecttargetdescriptorpayload.md)
- [EFFECT_RECEIPT_FORMAT](stove0-target-protocol-effect-receipt-format.md)
- [EFFECT_TARGET_PROTOCOL](stove0-target-protocol-effect-target-protocol.md)
- [EffectPlan](stove0-target-protocol-effectplan.md)
  - [binding_document](stove0-target-protocol-effectplan-binding-document.md)
  - [canonical_observation_results](stove0-target-protocol-effectplan-canonical-observation-results.md)
  - [seal](stove0-target-protocol-effectplan-seal.md)
  - [verify_digest](stove0-target-protocol-effectplan-verify-digest.md)
- [EffectPlanPayload](stove0-target-protocol-effectplanpayload.md)
  - [canonical_observation_results](stove0-target-protocol-effectplanpayload-canonical-observation-results.md)
- [ExternalEffectReceipt](stove0-target-protocol-externaleffectreceipt.md)
  - [bounded_result](stove0-target-protocol-externaleffectreceipt-bounded-result.md)
  - [seal](stove0-target-protocol-externaleffectreceipt-seal.md)
  - [verify_digest](stove0-target-protocol-externaleffectreceipt-verify-digest.md)
- [ExternalEffectReceiptPayload](stove0-target-protocol-externaleffectreceiptpayload.md)
  - [bounded_result](stove0-target-protocol-externaleffectreceiptpayload-bounded-result.md)
- [InputArtifact](stove0-target-protocol-inputartifact.md)
  - [canonical_path](stove0-target-protocol-inputartifact-canonical-path.md)
- [InputArtifactContract](stove0-target-protocol-inputartifactcontract.md)
  - [canonical_dispositions](stove0-target-protocol-inputartifactcontract-canonical-dispositions.md)
  - [validate_cardinality](stove0-target-protocol-inputartifactcontract-validate-cardinality.md)
- [InputDisposition](stove0-target-protocol-inputdisposition.md)
- [InputDispositionDeclaration](stove0-target-protocol-inputdispositiondeclaration.md)
- [JSON_SCHEMA_ONLY_SEMANTIC_PROFILE](stove0-target-protocol-json-schema-only-semantic-profile.md)
- [MAX_DEPARTURE_RESULT_BYTES](stove0-target-protocol-max-departure-result-bytes.md)
- [OperationContract](stove0-target-protocol-operationcontract.md)
  - [bind_semantic_conformance_vectors](stove0-target-protocol-operationcontract-bind-semantic-conformance-vectors.md)
  - [seal](stove0-target-protocol-operationcontract-seal.md)
  - [validate_roles](stove0-target-protocol-operationcontract-validate-roles.md)
  - [verify_digest](stove0-target-protocol-operationcontract-verify-digest.md)
- [OperationContractPayload](stove0-target-protocol-operationcontractpayload.md)
  - [bind_semantic_conformance_vectors](stove0-target-protocol-operationcontractpayload-bind-semantic-conformance-vectors.md)
  - [validate_roles](stove0-target-protocol-operationcontractpayload-validate-roles.md)
- [OutputArtifact](stove0-target-protocol-outputartifact.md)
  - [canonical_path](stove0-target-protocol-outputartifact-canonical-path.md)
- [OutputArtifactContract](stove0-target-protocol-outputartifactcontract.md)
  - [unique_roles](stove0-target-protocol-outputartifactcontract-unique-roles.md)
  - [validate_cardinality](stove0-target-protocol-outputartifactcontract-validate-cardinality.md)
- [OutputArtifactRoleCount](stove0-target-protocol-outputartifactrolecount.md)
- [OutputArtifactSetIdentity](stove0-target-protocol-outputartifactsetidentity.md)
  - [seal](stove0-target-protocol-outputartifactsetidentity-seal.md)
  - [seal_iterable](stove0-target-protocol-outputartifactsetidentity-seal-iterable.md)
  - [validate_summary](stove0-target-protocol-outputartifactsetidentity-validate-summary.md)
- [OutputCollectionRef](stove0-target-protocol-outputcollectionref.md)
- [OutputSourceEdge](stove0-target-protocol-outputsourceedge.md)
- [SHA256_PATTERN](stove0-target-protocol-sha256-pattern.md)
- [SemanticId](stove0-target-protocol-semanticid.md)
- [SemanticIntentConformanceVector](stove0-target-protocol-semanticintentconformancevector.md)
- [SemanticIntentConformanceVectors](stove0-target-protocol-semanticintentconformancevectors.md)
  - [canonical_vectors](stove0-target-protocol-semanticintentconformancevectors-canonical-vectors.md)
  - [covers_acceptance_and_rejection](stove0-target-protocol-semanticintentconformancevectors-covers-acceptance-and-rejection.md)
  - [sha256](stove0-target-protocol-semanticintentconformancevectors-sha256.md)
- [SemanticValidationProfile](stove0-target-protocol-semanticvalidationprofile.md)
  - [canonical_rules](stove0-target-protocol-semanticvalidationprofile-canonical-rules.md)
  - [seal](stove0-target-protocol-semanticvalidationprofile-seal.md)
  - [verify_digest](stove0-target-protocol-semanticvalidationprofile-verify-digest.md)
- [SemanticValidationProfilePayload](stove0-target-protocol-semanticvalidationprofilepayload.md)
  - [canonical_rules](stove0-target-protocol-semanticvalidationprofilepayload-canonical-rules.md)
- [Sha256](stove0-target-protocol-sha256.md)
- [TARGET_CALLBACK_HTTP_OPERATIONS](stove0-target-protocol-target-callback-http-operations.md)
- [TARGET_HTTP_OPERATIONS](stove0-target-protocol-target-http-operations.md)
- [TARGET_INPUT_PAGE_MAX](stove0-target-protocol-target-input-page-max.md)
- [TRANSFORM_TARGET_PROTOCOL](stove0-target-protocol-transform-target-protocol.md)
- [TargetCallbackAccess](stove0-target-protocol-targetcallbackaccess.md)
- [TargetCallbackAcknowledgement](stove0-target-protocol-targetcallbackacknowledgement.md)
- [TargetDeclaration](stove0-target-protocol-targetdeclaration.md)
- [TargetDescriptor](stove0-target-protocol-targetdescriptor.md)
  - [bind_result_kind](stove0-target-protocol-targetdescriptor-bind-result-kind.md)
  - [canonical_operations](stove0-target-protocol-targetdescriptor-canonical-operations.md)
  - [seal](stove0-target-protocol-targetdescriptor-seal.md)
  - [support_for](stove0-target-protocol-targetdescriptor-support-for.md)
  - [verify_digest](stove0-target-protocol-targetdescriptor-verify-digest.md)
- [TargetDescriptorPayload](stove0-target-protocol-targetdescriptorpayload.md)
  - [bind_result_kind](stove0-target-protocol-targetdescriptorpayload-bind-result-kind.md)
  - [canonical_operations](stove0-target-protocol-targetdescriptorpayload-canonical-operations.md)
- [TargetExecutionEvidence](stove0-target-protocol-targetexecutionevidence.md)
- [TargetFailure](stove0-target-protocol-targetfailure.md)
- [TargetInapplicable](stove0-target-protocol-targetinapplicable.md)
- [TargetInputAuthority](stove0-target-protocol-targetinputauthority.md)
  - [from_selection](stove0-target-protocol-targetinputauthority-from-selection.md)
  - [validate_summary](stove0-target-protocol-targetinputauthority-validate-summary.md)
- [TargetInputPage](stove0-target-protocol-targetinputpage.md)
  - [bind_page](stove0-target-protocol-targetinputpage-bind-page.md)
- [TargetInputRoleCount](stove0-target-protocol-targetinputrolecount.md)
- [TargetJobDeclaration](stove0-target-protocol-targetjobdeclaration.md)
  - [bind_execution](stove0-target-protocol-targetjobdeclaration-bind-execution.md)
  - [canonical_claim_id](stove0-target-protocol-targetjobdeclaration-canonical-claim-id.md)
- [TargetJobRequest](stove0-target-protocol-targetjobrequest.md)
  - [accepted](stove0-target-protocol-targetjobrequest-accepted.md)
  - [seal](stove0-target-protocol-targetjobrequest-seal.md)
  - [verify_digest](stove0-target-protocol-targetjobrequest-verify-digest.md)
- [TargetJobState](stove0-target-protocol-targetjobstate.md)
- [TargetJobStatus](stove0-target-protocol-targetjobstatus.md)
  - [canonical_derivation](stove0-target-protocol-targetjobstatus-canonical-derivation.md)
  - [validate_terminal_shape](stove0-target-protocol-targetjobstatus-validate-terminal-shape.md)
- [TargetOperationSupport](stove0-target-protocol-targetoperationsupport.md)
- [TargetOutputBinding](stove0-target-protocol-targetoutputbinding.md)
- [TargetOutputBindingSetIdentity](stove0-target-protocol-targetoutputbindingsetidentity.md)
- [TargetPlan](stove0-target-protocol-targetplan.md)
- [TargetPreflightRequest](stove0-target-protocol-targetpreflightrequest.md)
  - [canonical_observations](stove0-target-protocol-targetpreflightrequest-canonical-observations.md)
- [TargetPreflightResponse](stove0-target-protocol-targetpreflightresponse.md)
  - [bind_protocol](stove0-target-protocol-targetpreflightresponse-bind-protocol.md)
- [TargetProductionAuthority](stove0-target-protocol-targetproductionauthority.md)
  - [seal](stove0-target-protocol-targetproductionauthority-seal.md)
  - [verify_digest](stove0-target-protocol-targetproductionauthority-verify-digest.md)
- [TargetProductionAuthorityPayload](stove0-target-protocol-targetproductionauthoritypayload.md)
- [TargetProductionSealResponse](stove0-target-protocol-targetproductionsealresponse.md)
  - [validate_state](stove0-target-protocol-targetproductionsealresponse-validate-state.md)
- [TargetProgress](stove0-target-protocol-targetprogress.md)
  - [validate_total](stove0-target-protocol-targetprogress-validate-total.md)
- [TargetProtocol](stove0-target-protocol-targetprotocol.md)
- [TargetProtocolModel](stove0-target-protocol-targetprotocolmodel.md)
- [TargetResultKind](stove0-target-protocol-targetresultkind.md)
- [TargetRuntimeAuthority](stove0-target-protocol-targetruntimeauthority.md)
- [TargetSettlementAuthority](stove0-target-protocol-targetsettlementauthority.md)
  - [seal](stove0-target-protocol-targetsettlementauthority-seal.md)
  - [verify_digest](stove0-target-protocol-targetsettlementauthority-verify-digest.md)
- [TargetSettlementAuthorityPayload](stove0-target-protocol-targetsettlementauthoritypayload.md)
- [TransformPlan](stove0-target-protocol-transformplan.md)
  - [binding_document](stove0-target-protocol-transformplan-binding-document.md)
  - [canonical_observation_results](stove0-target-protocol-transformplan-canonical-observation-results.md)
  - [seal](stove0-target-protocol-transformplan-seal.md)
  - [verify_digest](stove0-target-protocol-transformplan-verify-digest.md)
- [TransformPlanPayload](stove0-target-protocol-transformplanpayload.md)
  - [canonical_observation_results](stove0-target-protocol-transformplanpayload-canonical-observation-results.md)
- [canonical_json_bytes](stove0-target-protocol-canonical-json-bytes.md)
- [canonical_json_sha256](stove0-target-protocol-canonical-json-sha256.md)
- [update_input_disposition_commitment](stove0-target-protocol-update-input-disposition-commitment.md)
- [update_output_artifact_commitment](stove0-target-protocol-update-output-artifact-commitment.md)
- [update_output_source_edge_commitment](stove0-target-protocol-update-output-source-edge-commitment.md)
- [update_target_output_binding_commitment](stove0-target-protocol-update-target-output-binding-commitment.md)
- [validate_declaration_against_operation](stove0-target-protocol-validate-declaration-against-operation.md)
- [validate_preflight_response_against_request](stove0-target-protocol-validate-preflight-response-against-request.md)
- [validate_status_against_request](stove0-target-protocol-validate-status-against-request.md)
