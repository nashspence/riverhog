# stove0-review-sampler-protocol: Python

[Atlas](../../../index.md) · [Authority](../index.md) · [Policies](../../../policies/index.md)

Declared public imports and their selected exact structural contracts.

## Semantic dossiers

### `stove0_review_sampler_protocol`

- [stove0_review_sampler_protocol.SAMPLER_HTTP_OPERATIONS](stove0-review-sampler-protocol-sampler-http-operations.md)
- [stove0_review_sampler_protocol.SAMPLER_PROTOCOL](stove0-review-sampler-protocol-sampler-protocol.md)
- [stove0_review_sampler_protocol.SamplerDescriptor](stove0-review-sampler-protocol-samplerdescriptor.md)
  - [stove0_review_sampler_protocol.SamplerDescriptor.seal](stove0-review-sampler-protocol-samplerdescriptor-seal.md)
  - [stove0_review_sampler_protocol.SamplerDescriptor.verify_digest](stove0-review-sampler-protocol-samplerdescriptor-verify-digest.md)
- [stove0_review_sampler_protocol.SamplerDescriptorPayload](stove0-review-sampler-protocol-samplerdescriptorpayload.md)
- [stove0_review_sampler_protocol.SamplerFailure](stove0-review-sampler-protocol-samplerfailure.md)
- [stove0_review_sampler_protocol.SamplerInapplicable](stove0-review-sampler-protocol-samplerinapplicable.md)
- [stove0_review_sampler_protocol.SamplerInput](stove0-review-sampler-protocol-samplerinput.md)
  - [stove0_review_sampler_protocol.SamplerInput.canonical_path](stove0-review-sampler-protocol-samplerinput-canonical-path.md)
- [stove0_review_sampler_protocol.SamplerOutput](stove0-review-sampler-protocol-sampleroutput.md)
  - [stove0_review_sampler_protocol.SamplerOutput.canonical_path](stove0-review-sampler-protocol-sampleroutput-canonical-path.md)
  - [stove0_review_sampler_protocol.SamplerOutput.canonical_sources](stove0-review-sampler-protocol-sampleroutput-canonical-sources.md)
- [stove0_review_sampler_protocol.SamplerRequest](stove0-review-sampler-protocol-samplerrequest.md)
  - [stove0_review_sampler_protocol.SamplerRequest.seal](stove0-review-sampler-protocol-samplerrequest-seal.md)
  - [stove0_review_sampler_protocol.SamplerRequest.verify_digest](stove0-review-sampler-protocol-samplerrequest-verify-digest.md)
- [stove0_review_sampler_protocol.SamplerRequestPayload](stove0-review-sampler-protocol-samplerrequestpayload.md)
  - [stove0_review_sampler_protocol.SamplerRequestPayload.canonical_cancellation_path](stove0-review-sampler-protocol-samplerrequestpayload-canonical-cancellation-path.md)
  - [stove0_review_sampler_protocol.SamplerRequestPayload.canonical_inputs](stove0-review-sampler-protocol-samplerrequestpayload-canonical-inputs.md)
  - [stove0_review_sampler_protocol.SamplerRequestPayload.canonical_windows](stove0-review-sampler-protocol-samplerrequestpayload-canonical-windows.md)
  - [stove0_review_sampler_protocol.SamplerRequestPayload.references_exact_inputs](stove0-review-sampler-protocol-samplerrequestpayload-references-exact-inputs.md)
- [stove0_review_sampler_protocol.SamplerResult](stove0-review-sampler-protocol-samplerresult.md)
  - [stove0_review_sampler_protocol.SamplerResult.seal](stove0-review-sampler-protocol-samplerresult-seal.md)
  - [stove0_review_sampler_protocol.SamplerResult.verify_digest](stove0-review-sampler-protocol-samplerresult-verify-digest.md)
- [stove0_review_sampler_protocol.SamplerResultPayload](stove0-review-sampler-protocol-samplerresultpayload.md)
  - [stove0_review_sampler_protocol.SamplerResultPayload.canonical_outputs](stove0-review-sampler-protocol-samplerresultpayload-canonical-outputs.md)
  - [stove0_review_sampler_protocol.SamplerResultPayload.state_shape](stove0-review-sampler-protocol-samplerresultpayload-state-shape.md)
- [stove0_review_sampler_protocol.SamplerWindow](stove0-review-sampler-protocol-samplerwindow.md)
  - [stove0_review_sampler_protocol.SamplerWindow.canonical_output_path](stove0-review-sampler-protocol-samplerwindow-canonical-output-path.md)
- [stove0_review_sampler_protocol.validate_result](stove0-review-sampler-protocol-validate-result.md)
