...
- **Archive authority.** Archive stores own encrypted bytes. Only sealed objects and published
  ...
  tools without Riverhog or its database.
...
- **Provenance authority.** Per-file provenance is append-only custody history. Journals remain
  exact prefixes across handoffs; omissions require a reason. Journals and their index are
  authoritative; database rows are a rebuildable projection.
...
- **Implementation ownership.** Riverhog owns product implementations and generic contracts.
  ...
  capabilities. They enter Riverhog only through public contracts.
...
- **Applications.** Applications use Riverhog capabilities and own their state;
  ...
  Delegated capabilities use distinct principal IDs. Catalog departure means exit from an
  authorization view; Stove0 seals that event for external effects and retains receipts.
- **Extensions.** Observers report immutable-artifact facts; targets perform declared operations.
  ...
  identities and selection; only exact digest-bound contracts or selected bindings carry authority.
