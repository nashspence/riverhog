...
def _target(*, create: bool = True) -> Path:
    raw = os.getenv("PIGGITY_LOCAL_ROOT", "").strip()
    if not raw:
        raise typer.BadParameter("PIGGITY_LOCAL_ROOT is required")
...
def _database(target: Path) -> Path:
    raw = os.getenv("PIGGITY_LOCAL_DATABASE", "").strip()
    return Path(raw).expanduser().resolve() if raw else target / ".piggity.sqlite3"
...
def _state_command(command: str, *, json_mode: bool) -> None:
    ...
    emit(
        ...
        if json_mode
            ...
            f"piggity local state: {payload['condition']} "
...
def _output_path(target: Path, collection_id: int, path: str) -> Path:
    ...
    if not output.is_relative_to(target):
        raise InvalidState("materialization path escapes PIGGITY_LOCAL_ROOT")
    return output
    ...
    *,
    ...
    for row in db.execute(
        ...
        if not repair:
            ...
            continue
        quarantine = target / ".piggity-quarantine" / str(row["collection_id"]) / row["path"]
...
def _download_job(
    ...
    transfer_root = target / ".piggity-transfers" / str(job["id"])
...
def list_collections(
    ...
    token_codec = BrowseTokenCodec(
        hashlib.sha256(b"piggity-local-list-token/v1\x00" + str(database).encode("utf-8")).digest(),
