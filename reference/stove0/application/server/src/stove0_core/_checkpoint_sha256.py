"""Private serializable SHA-256 state for bounded Stove0 work.

The checkpoint encoding is operational state, not a Stove0 protocol contract.
Completed identities are ordinary SHA-256 digests.
"""
...
class CheckpointSHA256:
    ...
    def export_state(self) -> str:
        return json.dumps(
            {
                "format": "stove0-private-sha256-checkpoint/v1",
                "h": self._h,
    ...
    def from_state(cls, state: str) -> Self:
        try:
            ...
            if payload.get("format") != "stove0-private-sha256-checkpoint/v1":
