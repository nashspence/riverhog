...
def _completion_precondition(
    ...
    return WriteCompletionPrecondition(
        ...
        state_token=hashlib.sha256(encoded).hexdigest(),
    )
...
class MemoryAdapter:
    ...
    def list_segments(self, request: WriteSegmentListRequest) -> WriteSegmentPage:
        ...
        return WriteSegmentPage(
            ...
            completion=(
                ...
                else _completion_precondition(all_segments)
            ),
        )
    ...
    def complete_write(
        ...
        if _completion_precondition(accepted) != request.completion:
            raise StorageAdapterRejection(
                "identity_conflict",
                "completion precondition differs from accepted fixture segments",
            )
        content = b"".join(
