...
def test_checkpoint_bootstrap_catchup_following_and_restart_are_separate_steps() -> None:
    ...
    deleted = CatalogSyncDeparture(cause="collection_deleted", collection_id="1", revision="92")
...
def test_empty_final_bootstrap_is_valid(last_id: int | None) -> None:
    result = sync._step(
        _Api(), _state(last_collection_id=str(last_id) if last_id is not None else None), limit=1
    )
...
def test_bootstrap_order_is_checked_within_and_across_pages(
    ...
    before = _roundtrip(_state(last_collection_id=str(last) if last is not None else None))
...
def test_change_progress_validation(
    ...
    api.changes = _changes(
        changes=[
            CatalogSyncDeparture(cause="collection_deleted", collection_id="1", revision=value)
...
def test_revision_upper_bound_is_lossless_and_visible_gaps_are_legal() -> None:
    ...
    api.changes = _changes(
        changes=[
            CatalogSyncDeparture(cause="collection_deleted", collection_id="1", revision=maximum)
        ],
...
def test_changed_response_authority_yields_no_observations(phase: str, field: str) -> None:
    ...
    api.changes = _changes(
        changes=[CatalogSyncDeparture(cause="collection_deleted", collection_id="1", revision="1")],
        through_revision="1",
        **{field: "9" * 64},
    )
...
def test_response_cannot_exceed_the_requested_page_bound(phase: str) -> None:
    ...
    api.changes = _changes(
        changes=[
            CatalogSyncDeparture(cause="collection_deleted", collection_id="1", revision=str(value))
            for value in (1, 2)
        ],
        through_revision="2",
    )
...
def test_change_retry_can_observe_a_later_unfixed_horizon() -> None:
    ...
    api.changes = _changes(
        changes=[
            CatalogSyncDeparture(cause="collection_deleted", collection_id="1", revision="11")
        ],
        through_revision="11",
    )
