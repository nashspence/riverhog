"""Private, persistence-free catalog traversal used by the public follower.

This module is not a supported durable-state format. Each call makes at most
one catalog request and returns observations plus a proposed state; it never
accepts its own result. CatalogReplica and external applications own storage.
...
class _SyncBatch:
    ...
    observations: tuple[CatalogSyncDescriptor | CatalogSyncUpsert | CatalogSyncDeparture, ...] = ()
    error: RiverhogError | None = None
    page: CatalogSyncCollectionPage | CatalogSyncChangePage | None = None
...
def _reset(
    state: _SyncState, reason: _ResetReason, *, error: RiverhogError | None = None
) -> _SyncBatch:
    return _SyncBatch(
        kind="reset",
        before=state,
...
def _start(api: _SyncApi, state: _SyncState) -> _SyncBatch:
    ...
    try:
        ...
        if exc.code in get_args(_ResetReason):
            return _reset(state, cast(_ResetReason, exc.code), error=exc)
        raise
...
def _catalog_page(state: _SyncState, page: CatalogSyncCollectionPage) -> _SyncBatch:
    ...
    for item in page.collections:
        if item.collection_id <= previous:
            raise ValueError("catalog collection order did not advance; page is not canonical")
    ...
    return _SyncBatch(
        ...
        page=page,
        next_state=_SyncState.model_validate(
            {
                "phase": "catalog" if page.next_cursor is not None else "catchup",
                "source_identity": state.source_identity,
                "authorization_view_identity": state.authorization_view_identity,
                "cursor": cursor,
                "last_collection_id": str(previous) if page.next_cursor is not None else None,
            }
        ),
    )
