...
from sqlite_example import StaleWorker, Store, initialize
...
class Chain:
    ...
    def anchor(self, height):
        ...
        return core.Anchor(height, hashlib.sha256(hashlib.sha256(self.header).digest())
                           .digest()[::-1].hex(), self.header, self.tip, "11" * 32)
...
def real_pending_job():
    ...
    return core.Job(REAL_DIGEST, NONCE, (A,), proof,
                    (core.Work("upgrade", A, REAL_DIGEST, due=100),))
...
@pytest.mark.parametrize("url", ["http://calendar.example", "https://u:p@calendar.example",
                                  "https://calendar.example/", "https://calendar.example?q=1",
                                  "https://calendar.example#x", "", "file:///x"])
...
@pytest.mark.parametrize("response", [b"", b"junk", b"x" * (core.MAX_PROOF + 1),
                                      mature_response() + b"trailing", REAL_PROOF])
...
@pytest.mark.parametrize("field,value", [("version", 2), ("version", True), ("nonce", "ab"),
                                        ("proof", ""), ("work", {}), ("submit_urls", A)])
...
@pytest.mark.parametrize("field,value", [("attempts", -1), ("due", True),
                                        ("kind", "delete"), ("commitment", "00" * 32),
...
def test_wrong_digest_and_trailing_bytes_fail_before_bitcoin_io():
    chain = Chain()
    for proof, digest in ((REAL_PROOF, DIGEST), (REAL_PROOF + b"x", REAL_DIGEST),
...
class RPC:
    ...
    def __call__(self, method, params):
        ...
        if method == "getblockchaininfo":
            ...
            return {"chain": self.network, "initialblockdownload": self.ibd,
                    "blocks": tip, "bestblockhash": "11" * 32}
...
def test_bitcoin_core_port_uses_active_height_raw_header_and_stable_tip():
    ...
    assert [method for method, _ in rpc.calls] == [
        "getblockchaininfo", "getblockhash", "getblockheader", "getblockhash",
        "getblockchaininfo"]


@pytest.mark.parametrize("options", [{"chain": "regtest"}, {"chain": "signet"},
                                      {"ibd": True}, {"changed": True}, {"corrupt": True}])
